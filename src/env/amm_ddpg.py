...
def train_agent(args: Config, USING_USD):
    ...
    gpu_id = 0
    market = GBMPriceSimulator(start_price=args.mkt_start, deterministic=True)
    ...
    buffer_items = agent.explore_env(env, args.horizon_len * 32, if_random=True)
...
def train_ddpg_for_amm(gpu_id=0):
    ...
    args.gamma = 0.0  # discount factor of future rewards
    args.fee_rate = 0.01
    args.epsilon = 0.005
    args.mkt_start = 1
