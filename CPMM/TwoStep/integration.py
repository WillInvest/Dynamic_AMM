...
class TwoStepIntegrate:
    def __init__(self, gamma: float, sigma: float, dt: float = 1.0, incoming_fee: bool = True):
        self.gamma = gamma
        self.sigma = sigma
        self.dt = dt
    ...
    def fee_integrand_uu(self, v2: float, v1: float) -> float:
        ...
        vv = v1 * v2
        assert abs(x1 * y1 - x0 * y0) < 1e-8, f"x1 * y1 != x0 * y0: {x1 * y1} != {x0 * y0}"
    ...
    def pv_integrand_uu(self, v2: float, v1: float) -> float:
        ...
        assert abs(x2 * y2 - x1 * y1) < 1e-8, f"x2 * y2 != x1 * y1: {x2 * y2} != {x1 * y1}"
        assert abs(x2 * y2 - x0 * y0) < 1e-8, f"x2 * y2 != x0 * y0: {x2 * y2} != {x0 * y0}"
...
if __name__ == "__main__":
    ...
    gamma_list = np.arange(0.0005, 0.9005, 0.0005)
