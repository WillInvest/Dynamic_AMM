...
def train(root_path, model_idx=None):
    ...
    envs = [lambda: Monitor(DummyAMM(market=MarketSimulator(sigma=0.15, seed=seed),
    ...
    model = PPO("MlpPolicy", env, verbose=2, learning_rate=0.0003, n_steps=3600)
    # if model_idx is None:
    #     model = TD3("MlpPolicy", env, verbose=2, learning_rate=0.0003, 
    #                 learning_starts=n_envs*36000,
    #                 train_freq=(n_envs, 'step'), gradient_steps=1)
    # else:
