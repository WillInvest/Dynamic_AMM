...
def train(root_path):
    ...
    for mc in np.arange(0.08, 0.11, 0.1):
        mc = round(mc, 2)
        wandb.init(project="AMM_RL_Trader_with_random_fee", entity='willinvest', name=f'mc_{mc:.2f}', config={"market_competence": mc})
        model_dirs = os.path.join(root_path, "models", f"market_competence_{mc:.2f}")
