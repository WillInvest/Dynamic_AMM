    ...
    for agent_seed in range(SEED_LEN):
        for fee_rate in fee_rates:
            for sigma in sigmas:
                ...
                wandb.init(project='AMM_AGENT_TRAIN',
                           config=config,
                           name=f"{MODEL}{agent_seed}-f{fee_rate}-s{sigma}-{eval_steps}")
                ...
                envs = [lambda: Monitor(ArbitrageEnv(market=MarketSimulator(sigma=sigma), amm=AMM(fee=fee_rate)), filename=None) for _ in range(10)]
                ...
                for _ in range(int(TOTAL_STEPS // EVALUATE_PER_STEP)):  # Adjust the range based on your needs
                    ...
                    eval_market = MarketSimulator(deterministic=eval_deterministic, steps=eval_steps)
                    eval_env = Monitor(ArbitrageEnv(market=eval_market, amm=AMM(fee=fee_rate)))
...
if __name__ == "__main__":
    ...
    rates = np.arange(0.21, 0.40, 0.01)
