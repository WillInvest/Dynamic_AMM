...
class TwoStepIntegrate:
    ...
    def fee_integrand_u(self, v1: float) -> float:
        ...
        """
        w_ratio = (1-self.w) / self.w
        S = self.p0 * v1
        if self.incoming_fee:
            delta_y = 1/(1-self.gamma) * (self.L * (w_ratio * (1-self.gamma) * S)**self.w - self.y0)
            return self.gamma * delta_y * self.dist.pdf(v1)
        else:
            delta_x = (self.x0 - self.L * (w_ratio * (1-self.gamma) * S)**(self.w-1))
            return self.gamma * delta_x * S * self.dist.pdf(v1)
    ...
    def fee_integrand_d(self, v1: float) -> float:
        ...
        S = self.p0 * v1
        if self.incoming_fee:
            delta_x = 1/(1-self.gamma) * (self.L * (w_ratio / (1-self.gamma) * S)**(self.w-1) - self.x0)
            return self.gamma * delta_x * S * self.dist.pdf(v1)
        else:
            delta_y = (self.y0 - self.L * (w_ratio / (1-self.gamma) * S)**self.w)
            return self.gamma * delta_y * self.dist.pdf(v1)
    ...
    def fee_integrand_uu(self, v2: float, v1: float) -> float:
        ...
        x0 = self.x0
        y0 = self.y0
        x1 = self.L * (w_ratio * (1-self.gamma) * S1)**(self.w-1)
        y1 = self.L * (w_ratio * (1-self.gamma) * S1)**self.w
        assert abs(x1**self.w * y1**(1-self.w) - x0**self.w * y0**(1-self.w)) < self.eps, f"x1 * y1 != x0 * y0: {x1 * y1} != {x0 * y0}"
        if self.incoming_fee:
            delta_y = 1/(1-self.gamma) * (self.L * (w_ratio * (1-self.gamma) * S2)**self.w - y1)
            assert delta_y >= 0, f"delta_x is negative: {delta_y}"
            return self.gamma * delta_y * self.dist.pdf(v1) * self.dist.pdf(v2)
        else:
            delta_x = (x1 - self.L * (w_ratio * (1-self.gamma) * S2)**(self.w-1))
    ...
    def fee_integrand_ud(self, v2: float, v1: float) -> float:
        ...
        x1 = self.L * (w_ratio * (1-self.gamma) * S1)**(self.w-1)
        y1 = self.L * (w_ratio * (1-self.gamma) * S1)**self.w
        assert abs(x1**self.w * y1**(1-self.w) - x0**self.w * y0**(1-self.w)) < self.eps, f"x1 * y1 != x0 * y0: {x1**self.w * y1**(1-self.w)} != {x0**self.w * y0**(1-self.w)}"
        if self.incoming_fee:
            delta_x = 1/(1-self.gamma) * (self.L * (w_ratio / (1-self.gamma) * S2)**(self.w-1) - x1)
            assert delta_x >= 0, f"delta_x is negative: {delta_x}"
            return self.gamma * delta_x * S2 * self.dist.pdf(v1) * self.dist.pdf(v2)
        else:
            delta_y = (y1 - self.L * (w_ratio / (1-self.gamma) * S2)**self.w)
    ...
    def pv_integrand_uu(self, v2: float, v1: float) -> float:
        ...
        x1 = self.L * (w_ratio * (1-self.gamma) * S1)**(self.w-1)
        y1 = self.L * (w_ratio * (1-self.gamma) * S1)**self.w
        x2 = self.L * (w_ratio * (1-self.gamma) * S2)**(self.w-1)
        y2 = self.L * (w_ratio * (1-self.gamma) * S2)**self.w
...
if __name__ == "__main__":
    ...
    path = '/Users/haofu/Desktop/AMM/Dynamic_AMM/G3M/two_step/results'
